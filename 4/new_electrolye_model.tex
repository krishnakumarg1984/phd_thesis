...
\subsubsection*{Transfer function identification of electrolyte time-evolution subsystem}\label{subsubsec:sysidplantmodel} % might possiblly have to promote into its own detailed subsection
...
moles of \ch{Li^+} per unit area  in each of the three regions $Q_{\text{e,}_j}$
...
\begin{enumerate}
    ...
    \item Multi-sine signal \ie{} a signal consisting of sinusoids at
        various fundamental frequencies added together (1000--\SI{1199}{\second})
\end{enumerate}
...
time-domain appearance. The specific validation current profile used in this system
identification is shown in~\cref{fig:sysidvalidationcurrent}
...
% code snippet for both training and testing input current
