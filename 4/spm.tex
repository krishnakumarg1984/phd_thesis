...
\subsection{Model Development --- Scope and Assumptions}\label{subsec:basicspmassumptions}
...
\subsection[Model   Development  ---   Governing  Equations]{Model   Development
---   Governing   Equations\protect\footnote{In   this   section,   only   those
simplifications to  mathematical notations arising due  to assumptions discussed
in~\cref{subsec:basicspmassumptions}   shall  be   introduced  in-line.   For  a
comprehensive  reference to  the notations  used, please  refer to  nomenclature
list in the~\nameref{ch:glossary} chapter.  Notations introduced solely for
this section are also covered here.}}\label{subsec:basicspmgoverningeqns}
...
becomes
\begin{align}
    j_\nj(t) &= \pm \frac{I(t)}{A l_j a_\sj F}\label{eq:uniformcurrdensity}
    \shortintertext{Substituting~\cref{eq:uniformcurrdensity} in~\cref{eq:csfluxsurface},}
    D_\sj\diffp{c_\sj}{r}{r=R_\pj} &= \mp \frac{I}{A l_j a_\sj F}\label{eq:csfluxsurfacefinal}
\end{align}
...
\subsection{Model Formulation --- Further Reduction in Dimensionality}\label{subsec:furtherdimensionalityreduction}
...
Given  the   elaborate  simplifications   made  to  remove   spatial  resolution
from  the  axial  direction,  the  efficacy of  using  a  radial  discretisation
is  rendered  questionable,  particularly  within the  scope  of  embedding  the
model  in an  online  simulation and  state-estimation  environment. Given  that
diffusion  in  each  spherical  particle   is  modelled  by  well-known  Fickian
dynamics,   several  attempts   have  been   made  to   obtain  an   approximate
analytical  solution  for the  solid  phase  concentration in  both  electrodes.
...
was  performed  by Santhanagopalan~\etal~\cite{Santhanagopalan2006}.  Zhang  and
White~\cite{Zhang2007}  provide   a  comparative   evaluation  of   the  various
approximation methods in  a dimensionless analysis study, a summary  of which is
presented in~\cref{tbl:solidphaseapprox}
