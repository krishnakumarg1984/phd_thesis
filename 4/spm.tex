...
For control  oriented applications,  it is  imperative to write  the model  in a
classical  state  representation  along  the  lines  of~\cref{eq:LTIstatespace}.
However,   the   non-linearities   in   the  equation   for   terminal   voltage
(see~\cref{eq:cellterminalvoltagebasic})  imply  that  it  is  not  possible  to
represent the \gls{spm} as  the classical LTI system of~\cref{eq:LTIstatespace}.
Instead, the system is  represented by a system of state  equations along with a
