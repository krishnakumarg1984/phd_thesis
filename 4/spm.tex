...
\footnotetext{For the sake of brevity, in rest of the equations in this section,
    the explicit definition of the subscripted variable $j$ shall be omitted. It
    is implied that \jinnegpos{} since these equations describe solid phase
diffusion in the negative and positive electrode.}
The divergence of a vector field $\mathbf{F}(r,\theta, \phi)$ can be expressed in spherical
co-ordinates as
...
\begin{equation}\label{eq:csdiffusioneqn}
    \diffp{c_\sj}{t} = \frac{1}{r^2}\diffp*{r^2 \left(D_\sj\, \nabla c_\sj \right)}{r}
\end{equation}
\Cref{eq:csdiffusioneqn} represents the mass-balance equation describing solid
...
density $j_n(x,t)$ is eliminated, thus possessing only temporal dependence and
denoted by $j_n(t)$. Hence, at the particle surface
\begin{equation}\label{eq:csfluxsurface}
    \diffp{c_\sj}{r}{r=R_\pj} = -j_\jn(t)
\end{equation}
The sign convention chosen here is such that pore-wall flux leaving the particle
surface is considered to be positive.

Charge  conservation  in  solid  phase  is applied  in  order  to  evaluate  the
\gls{rhs}  in~\cref{eq:csfluxsurface}.  The  detailed derivation  which  employs
the  assumptions listed  in~\cref{subsec:basicspmassumptions},  is presented  in
Domenico~\etal~\cite{DiDomenico2010}.
