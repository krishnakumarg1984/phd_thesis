...
\begin{equation}\label{eq:csfluxsurface}
    D_\sj\diffp{c_\sj}{r}{r=R_\pj} = \frac{-j_\nj}{a_\sj F}
\end{equation}
...
\begin{align}
    j_\nj(t) &= \frac{I(t)}{l_j
        A}\label{eq:uniformcurrdensity}\shortintertext{Substituting~\cref{eq:uniformcurrdensity}
    in~\cref{eq:csfluxsurface},}
    D_\sj\diffp{c_\sj}{r}{r=R_\pj} &= \frac{I}{A l_j a_\sj
    F}\label{eq:csfluxsurfacefinal}
\end{align}
In~\cref{eq:csfluxsurfacefinal}, the explicit time-dependence of the load current $I(t)$
...
\Cref{eq:csdiffusionfinal},   its  boundary   conditions~\eqref{eq:csfluxcentre}
\&~\eqref{eq:csfluxsurfacefinal}           along          with           initial
condition~\eqref{eq:csfluxinitialcondition}  provide  the  complete  description
of  time-domain  evolution  of  lithium  in the  conventional  \gls{spm}  for  a
given applied  current profile  $I(t)$. Certain  aspects of  efficient numerical
simulation of this system is presented next in~\cref{subsec:basicspmgeometry}

\subsection{Model Formulation --- Further Reductions in Dimensionality}\label{subsec:furtherdimensionalityreduction}
