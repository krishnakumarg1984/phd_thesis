...
\noindent\makebox[\textwidth][c]{%
    \begin{customalgo}[1.0\textwidth]{Continuous time \gls{spm}}{alg:ctstimespm}
        ...
        \State {$t_\text{f,desired} =
            ...
            &\text{%\scriptsize
        otherwise.}
    \end{cases}$} \Comment{\parbox[t]{0.275\textwidth}{May terminate early due to violation of cut-off conditions}}
% }}
        ...
        \Comment{Max number of iterations needed assuming no cut-offs}
        ...
        \State $I[0] \gets I_\text{1C} \times  \text{C-rate}[0], \mathbf{x}[0] \gets \vect{0,0, \mean{c}_\sneg[0]}$ \Comment{ $\text{C-rate}[0]$ from profile}
        ...
        \Comment{Voltage change from direct feedthrough}
        ...
        \Comment{with its initial condition set to $x[k-1]$}
        \State $\mathbf{x}[k] \gets $ last time-entry  vector of
        soln.\  matrix
        \Comment{from an adaptive solver like MATLAB's \texttt{ode45}}
        \State Compute $z[k]$ as per~\cref{eq:soccomputation}
        \State $V_\text{cell} \gets \textsc{ComputeCellVoltage}(\textbf{x}[k],I[k],\texttt{param}) $
        \If $z[k] \text{or} V_\text{cell}[k]$ outside limits \Comment{check if
        voltage of \gls{soc} exceeded upper or lower cutoffs}
            \State $k \gets k - 1$ \Comment{data from last simulation step is invalid}
            \State \textit{break};
            \EndIf
        \EndFor
        \EndProcedure
        ...
        \Comment{Implement saturation \ie{} $c_\snegmin \le c_\snegsurf \le
        c_\snegmax$}
