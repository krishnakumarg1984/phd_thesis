...
time. As  seen in~\cref{tbl:charSimspmp2d}, even the  \emph{combined} simulation
time  of  the \gls{p2d}  and  \gls{spm}  models  is  lower than  real-time.  The
only  persistent issue  here  is that  of memory  and  storage requirements  due
to  high-rate  data-logging for  accurate  coulomb  counting. Nevertheless,  for
comparable  volume  of data  to  be  logged,  reliability  and ruggedness  of  a
dedicated  computer  workstation far  exceeds  that  of real-time  cell  cyclers
thereby facilitating the desired numerical  characterisation of cell capacity. A
major  disadvantage  of simulation  based  capacity  validation is  its  extreme
sensitivity to  parameters such as  the maximum concentration of  the electrodes
and their  stoichiometries, which  are generally  difficult to  characterise. In
this  context, the  experimental  procedure of  high-precision coulomb  counting
assumes  a practical  significance as  it requires  no knowledge  of the  cell's
parameters other than the manufacturer's datasheet.
...
implementation  from~\cref{sec:numericalimplementation}. The error vector is
defined as
\begin{equation}
    \varepsilon = V_{\text{cell}_\text{p2d}} - V_{\text{cell}_\text{spm}}
\end{equation}
