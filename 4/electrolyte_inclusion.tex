...
average ionic  concentration in each of  the three cell regions  \viz{} positive
electrode, separator and negative electrode are described through a set of three
\glspl{ode}. As  already established by  the discussion thus far,  the gradients
along  the  axial  thickness  of  the cell  is  significant,  even  at  moderate
{C-rates}. Lending strength to doubts  in the model's applicability in scenarios
with  electrolyte concentration  gradients  is the  fact  that constant  current
results have not been  reported in this work. Thus, this model  is deemed as not
satisfactory to warrant further engagement.
