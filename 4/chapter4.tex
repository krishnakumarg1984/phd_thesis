...
enumerate in-line here} It is important to distinguish between models that are derived directly in the time domain
versus those that are derived first in the frequency domain and later converted to time domain.\todo{write this better}
...
model from Smith~\etal{}~\cite{Smith2007}.\todo{write plett's group work in the next couple of paragraphs explaining the
    DRA. DRA was a way forward in bringing all the internal quantities closer to time-domain implementation rather than
resorting to a lumped impedance parameter}
