...
% classical \gls{dra}, the second step in the process, i.e. forming the Block-Hankel
...
% 	    If the \gls{svd} operation can be performed on a virtual Hankel matrix,
...
% 	    (zeros) in memory.
% 	\item It is not necessary to perform a \textit{full} \gls{svd} operation in order
...
% 	    \gls{svd} yielding the first few dominant triplets of $U,\text{ \ensuremath{\Sigma}\ \text{and }}V$.
% \end{enumerate}
% Thus, forming the large Block-Hankel matrix and computing its \gls{svd}
% is identified as an avoidable bottleneck in the classical \gls{dra}method.
...
% limits. Facilitated by an efficient \gls{svd} implementation, we propose
% an improved \gls{dra}that serves as a drop-in replacement in the \gls{rom} workflow.
