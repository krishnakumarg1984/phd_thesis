...
symbolic  data  mining  and  is  ideally suited  for  such  symbolic  regression
tasks (fitting  a mathematical  curve, and  not just  fitting coefficients  to a
pre-assumed curve as in classical numerical regression, to a given data).

It is  important to recognize  that the key  criteria that restricts  the choice
of  gene  sequence  depth  as  well  as  parent  mutations  to  choose  from  is
the  total  number  of  unknown  symbolic coefficients  assumed  by  the  model.
Looking at the  spatial concentration profile from  \gls{p2d} simulation results
shown in~\cref{fig:spatialionicconc1C},  it is evident that  a simplified family
of  curves  may  be used  for  the  separator  region,  relative to  those  used
for  the concentration  within  the  electrode regions.  There  are  a total  of
...
This process of symbolic regression
did not yield any improvements, in particular, no single mathematical formula
