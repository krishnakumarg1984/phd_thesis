...
\begin{align}
    \mathbf{x}(t) &= e^{A (t-t_0)}\mathbf{x}(t_0) + \int_{t_0}^{t}e^{A (t-τ)}\mathbf{B u}(τ)\,dτ \label{eq:genanalyticctssoln}
    \\
...
\end{alignat}
% The dummy  variable of  integration $\tau$  can similarly  be transformed  to an
% arbitrary  multiple  of  $T_s$  whose  value is  applicable  within  one  sample
% period, \ie{}  $\mathbf{u}(\tau) =  \mathbf{u}(\kappa T_s)$.
Substituting these relationships into~\cref{eq:genanalyticctssoln},
\begin{equation}
    \mathbf{x}[k+1] = e^{A T_s}\mathbf{x}[k] + \int_{k T_s}^{(k+1)T_s}e^{A ((k+1)T_s-τ)}\mathbf{B u}(τ)\,dτ \label{eq:intermediatediscrete}
\end{equation}
...
\begin{align}
    \mathbf{x}[k+1] &= e^{A T_s}\mathbf{x}[k] + \left[\int_{T_s}^{0}e^{A \eta }\mathbf{B} \right] u[k]\,{-d\eta}\\
    \shortintertext{Reversing the order of integration leads to}
    \mathbf{x}[k+1] &= e^{A T_s}\mathbf{x}[k] + \left[\int_{0}^{T_s}e^{A \eta }\mathbf{B} \,{d\eta} \right] u[k] \label{eq:disctimefulleqn}
\end{align}
...
\end{equation}
where $A_d = e^{A T_s}$ and $B_d = \int_{0}^{T_s}e^{A \eta}\mathbf{B}
\,{d\eta}$.
...
\begin{bmatrix}
    ...
    0                                    & 0                                    & 1
\end{bmatrix}\label{eq:A_d}
% \begin{bmatrix}
%     e^{-30\frac{D_\spos}{R_\ppos^2} T_s} & 0                                                 & 0 \\
%     0                                                & \exp\left(-30\frac{D_\sneg}{R_\pneg^2} T_s\right) & 0 \\
%     0                                                & 0                                                 & \exp\left(0 T_s\right)
% \end{bmatrix}
\end{alignat}
\begin{align}
    B_d &= \int_{0}^{T_s}e^{A \eta}\mathbf{B} \,{d\eta} \\
...
algorithms.  Further analysis  in discrete-time  such as  aliasing, quantisation
noise, signal pre-conditioning  and discrete-time fourier analysis  falls in the
specialised engineering domain of signal processing  and is outside the scope of
the thesis.  Thus, a discrete-time  model of  the basic ~\gls{spm}  is available
whose results are presented next in~\cref{sec:numericalimplementation}.


% mention that output equation has not really changed, but still give the
% equation in terms of indices
% Table comparing simulation speeds for cts and disc for constant current and dynamic current

\todo{DT algo here along with a listing of matlab code in appendix}
