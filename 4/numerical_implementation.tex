...
and needs to be solved numerically  with some variant of a Newton-type iteration
scheme.} is given by
\begingroup
\allowdisplaybreaks
...
\begin{figure}[tbh]
    ...
    \framebox[\textwidth]{
        \begin{subfigure}[t]{\wd\algboxA}
            \subcaption{background process (low priority)}\label{subfig:bgRTprocess}
            \usebox{\algboxA}
        \end{subfigure}
        \hfill
        \begin{subfigure}[t]{\wd\algboxB}
            \subcaption{foreground processes (high priority)}\label{subfig:fgRTprocess}
            \raisebox{\dimexpr.5\ht\algboxA-.5\ht\algboxB}{%
                \usebox{\algboxB}%
            }%
        \end{subfigure}
