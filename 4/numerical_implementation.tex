...
The discrete-time variant of the \gls{spm} is given in
\input{4/algo_disctime_spm}

Further  analysis  in  discrete-time   such  as  aliasing,  quantisation  noise,
signal  pre-conditioning  and  discrete-time   fourier  analysis  falls  in  the
specialised  engineering  domain  of  signal   processing  and  is  outside  the
scope  of the  thesis.  Thus,  a discrete-time  model  of  the basic  ~\gls{spm}
is  now  available. The  results  of  the  basic  \gls{spm} are  presented  next
in~\cref{sec:basicspmsimresults}.
