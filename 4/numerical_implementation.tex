...
\begin{listing}
\begin{minted}[mathescape,autogobble,bgcolor=mintedbg,escapeinside=||,label=Returns,labelposition=topline]{matlab}
% Returns $A_d$ and $B_d$ matrices
A_cts = [-30*Ds_pos/(R_pos^2),                    0, 0; ...
                            0, -30*Ds_neg/(R_neg^2), 0; ...
                            0,                    0, 0];
% $A_d = e^{A T_s}$
|see \crefrange{eq:A_dinit}{eq:A_d}|
A_disc = expm(A_cts*Ts); % $\mathtt{expm}$ command returns matrix exponential
...
% $B_d = \int_{0}^{T_s}e^{A \eta}\mathbf{B} \,{d\eta}$
|see \crefrange{eq:B_dinit}{eq:B_d}|
