...
\begin{center}
    \begin{customalgo}[0.9\textwidth]{Continuous time \gls{spm}}{alg:ctstimespm}
        ...
        \Userdata $\text{\gls{soc}}(0), t_\text{f,user}$, $t_\text{f,condition}$ \Comment{$t_\text{f,condition} \in  \left\{\texttt{max}, \texttt{min}\right\}$}
        \Userdata Cell capacity $I_\text{1C}$, Sample rate $T_s$
        \Ensure \gls{soc}(0), $v(0)$ within limits
        \Procedure{Simulate\gls{spm}}{}
        \State {$t_\text{f,desired} =
                                ...
                                \begin{cases}
                                    ...
                                    &\text{%\scriptsize
                                otherwise.}
                            \end{cases}$} %\Comment{Extrapolate last C-rate from profile until $t_\text{f,desired}$ if necessary}
