...
\begin{table}[!htbp]
    \small
    \caption[Simulation parameters of an \protect{\gls{lco}} cell]{Complete set of parameters for simulating the \protect{\gls{p2d}} and \protect{\gls{spm}} implementations of an \protect{\gls{lco}} cell (with \ch{LiCoO_2}--\ch{LiC_6} electrode pair and \ch{LiPF_6} electrolyte),\quad \protect{$j \in \{\text{pos},\text{sep},\text{neg}\}$}}
    ...
    \begin{threeparttable}
        ...
        \begin{tabularx}{\textwidth}{ l L C R }
            ...
            \rowcolor{imperiallightgray} Electrolyte diffusivity, $D_j$ (\si{\meter\squared\per\second})                         & \tnote{d}\num{3.22e-10} & \tnote{d}\num{3.22e-10}                           & \tnote{d}\num{3.22e-10} \\
            \rowcolor{imperiallightgray} Electrolyte conductivity, $\kappa_j$ (\si{\siemens\per\meter})                          & \tnote{e}\num{26.24e-3} & \tnote{e}\num{328.15e-3}                          & \tnote{e}\num{66.08e-3} \\
            \rowcolor{imperiallightgray} \ch{Li^+} transference number, $t^0_\text{+}$                                           & \tnote{c}\num{0.363}    & \tnote{c}\num{0.363}                              & \tnote{c}\num{0.363}    \\
            \rowcolor{imperiallightgray} Electronic conductivity, $\sigma_j$ (\si{\siemens\per\meter})                           & \tnote{c}\num{100.00}   & ---                                               & \tnote{c}\num{100.00}   \\
                                         Thickness, $l_j$ (\si{\meter})                                                          & \tnote{c}\num{88e-6}    & \cellcolor{imperiallightgray}\tnote{c}\num{25e-6} & \tnote{f}\num{72e-6}    \\
        ...
        \item[d] Computed at $T_\text{cell} = \SI{298.15}{\kelvin}$ using coefficients from table \rom{2} in Ref.~\cite{Valoen2005} \\
