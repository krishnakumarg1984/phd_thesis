...
# If from windows, run this script from only cmder's version of cmd/powershell (not from native cmd, native powershell, mintty-cygwin, mintty-mingw or git-bash). This is because call to xindy shall fail otherwise
if [ ! -d ../nonflat_submitted ]; then
    ...
    # git checkout 0f26b11 # without footer.  The -f flag helps to force checkout to facilitate local uncommitted changes to the repo (like testing this script)
    cd ../phd_thesis/
fi
...
if [ ! -d ../diff_head_submitted ]; then
    ...
    if [ "$(expr substr $(uname -s) 1 10)" == "MINGW32_NT" ] || [ "$(expr substr $(uname -s) 1 10)" == "MINGW64_NT" ] || [ "$(expr substr $(uname -s) 1 7)" == "MSYS_NT" ]; then
        H:\portable_win_utilities\sed.exe -i "s/main/diff/g" ..\diff_head_submitted\latexmkrc
    else
