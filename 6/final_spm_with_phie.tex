...
\input{6/final_spm_1C_error_table}
...
\input{6/final_spm_udds_error_table}

In  summary, while  the newly  developed composite  \gls{spm} is  not considered
robust  for  use  in  sustained  discharge  at high  C-rates,  it  is  indeed  a
suitable option  for dynamic load  profiles, even  with high peak  magnitudes of
...
has been developed through system identification. The shortcomings of this model
has been  quantified. However,  its usefulness,  accuracy and  applicability has
been proven  for real-life scenarios  using a  highly dynamic load  cycle. Thus,
this  author  concludes  that  the composite  \gls{spm}  developed  here  indeed
represents  an  advancement  to  the  modelling  art.  The  composite  \gls{spm}
developed here may  be used as the  plant model in state  estimation and control
applications in a vehicular \gls{bms}. This concludes the implementation aspects
