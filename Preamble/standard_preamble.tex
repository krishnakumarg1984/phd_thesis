%!TEX root = ../main.tex
% vim:nospell

\usepackage{algpseudocode}% http://ctan.org/pkg/algorithmicx
\usepackage{amsfonts}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{anyfontsize}
\usepackage[title,titletoc]{appendix} % add appendices

% \usepackage[backend=biber, style=ieee, sortlocale=en_GB, maxbibnames=50, url=true, doi=true, eprint=true ]{biblatex}
\usepackage[backend=biber, style=numeric-comp, sorting=none, citestyle=numeric-comp, maxbibnames=50, url=true, doi=true, eprint=false,backref=true,backrefstyle=three]{biblatex} % seems to have strong dependence on csquotes?
\usepackage{bigints}
\usepackage{booktabs}

\usepackage[makeroom]{cancel}
\usepackage[margin=10pt,font=small,labelfont={bf},labelsep=quad]{caption} % for improved layout of figure captions with extra margin, smaller font than text
\usepackage{checkend}
\usepackage{chkfloat}
% \usepackage{cite} % incompatible with biblatex
% \usepackage{cmdtrack}
% \usepackage{colortbl} % colortbl cannot be used if xcolor is used

\usepackage{diffcoeff}
% \usepackage{dcolumn}

\usepackage[normal]{engord}
\usepackage[inline]{enumitem}
% \usepackage{etoolbox} % not really required if using glossaries package, since this package then gets loaded automatically

\usepackage{fancyhdr} % Define custom header (before hyperref), need to see how pdfx package affects the loading order
\usepackage[draft]{fixme}
\usepackage{flafter}
%\usepackage{fnpct}
% \usepackage{footnote}
\usepackage[bottom]{footmisc}

\usepackage[luatex,paper=a4paper,hmarginratio=1:1, vmarginratio=1:1, scale=0.75]{geometry}
\usepackage[local,mark]{gitinfo2}
\usepackage{graphicx} % important to load before fontspec

\usepackage{labelschanged}
% \usepackage{layouts} % helpful for computing textwidth, textheight etc
% \usepackage{lettrine}
\usepackage{lualatex-math}
% \usepackage{luabibentry}

\usepackage{makecell}
\usepackage{mathtools}
\usepackage[frac,vfrac,multskip]{mathfixs}
% \usepackage[activate={true,nocompatibility},final,tracking=true,factor=1100,stretch=10,shrink=10]{microtype}
\usepackage{multicol}
\usepackage{multirow}

% \usepackage{nccmath}
% \usepackage{needspace}
% \usepackage{nolbreaks}

% \usepackage[all,warning]{onlyamsmath} % if using Tikz, please include the calc and babel libraries (known incompatibilities)

\usepackage{pdfpages}
\usepackage[section]{placeins} %Defines a \FloatBarrier command

\usepackage{setspace} % Define line spacing

% \usepackage{blindtext}
% \usepackage{showframe}
% \usepackage[noframe]{showframe}
\usepackage[separate-uncertainty = true,multi-part-units=single,detect-all]{siunitx}
% \usepackage{soulutf8}
\usepackage{subcaption}
% \usepackage{subfiles}

% \usepackage{tablefootnote}
\usepackage{tabstackengine}
% \usepackage{tabularx}
\usepackage{titlesec}
\usepackage{titletoc}
\usepackage{threeparttable}
\usepackage[nottoc]{tocbibind} % correct page numbers for bib in TOC, nottoc suppresses an entry for TOC itself

\usepackage[normalem]{ulem}
\usepackage{url}

\usepackage{varwidth}

\usepackage{witharrows}

% \usepackage[table]{xcolor} % loaded by pdfx package (if used) cannot explicitly load colortbl package either before or after
\usepackage{xfrac}
\usepackage{xpatch} % helpful to replace parens with brackets for backreferencing with biblatex

