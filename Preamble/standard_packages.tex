...
% applicable only for  luatex. Most notably, microtype is missing  here since it
% needs to be loaded after babel (which is to be loaded after fontspec)
...
\usepackage{datetime}
\usepackage{datetime2}   % must be loaded before gitinfo2
...
\usepackage{geometry}
\usepackage{gitinfo2}    % if using datetime2, that package must be loaded before gitinfo2
...
\usepackage{xfrac}
\usepackage{xpatch}      % an example of usage is to replace parens with brackets for backreferences (in bibliography) with biblatex
