...
\subsection{Initial set of assumptions}\label{subsec:layeroptassumptions}

To obtain a balanced loading of both electrodes and to avoid early exhaustion of
lithium  from  one of  the  electrodes  during  operation,  it is  desirable  to
carefully  calculate  the  volume  of electrochemical  active  materials  to  be
accommodated  within  the cell.  This  concept  is  well-known and  is  commonly
discussed  in  standard  textbooks in  the  field  such  as  those by  Rahn  and
Wang~\cite{Rahn2013}  wherein  example  calculations  are  presented  for  block
...
Yet another  redeeming argument to  support the constant porosity  assumption is
from  a technical  viewpoint. Keeping  material porosities  constant enables  to
eliminate a degree of freedom in  the design optimisation, thereby narrowing the
dimensionality of the search space. To the best of the author's knowledge, there
has not yet been any published  work tackling layer optimisation of pouch cells.
Obtaining an  initial set of  results through a constant  porosity approximation
shall provide a solid foundation to build upon. The author therefore sees a high
research value in  obtaining quick ballpark estimates of an  optimal layer count
with constant porosities.  Nevertheless, the influence of  varying porosities in
the design optimisation process is to be acknowledged as an important element in
future studies.

Another assumption  in the formulation  of the optimisation methodology  is that
the pack configuration (series/parallel arrangement  of modules, number of cells
per module and other system-level  specifications) are held constant throughout.
This validity of this assumption is easily justified since the cell-level design
may  be  performed independently  of  the  larger  drivetrain design.  In  fact,
the  author  postulates  that  present  empirical  cell  designs  are  based  on
specifications  laid out  by  vehicle manufacturers  and does  not  adhere to  a
model-oriented integrated  design with the complete  drivetrain. This modularity
in the  design approach enables  to keep such system-level  parameters constant.
Yet another assumption in this study is  that the overall height of the pouch is
held constant. Without such constraints, any arbitrary pouch size can be chosen,
leading  to an  infinite-dimensional  optimisation problem  wherein no  definite
optimality criterion exists. This assumption is in-fact enforced by the trend in
industry \viz{} common-module design wherein the physical dimensions of the pack
are chosen  a~priori and modularisation of  pack helps in tailoring  of packs to
cater to different market segments. Extrapolating this common module design down
to the cell level, it is justifiable to have cells of same height.
...
used. Barring  minor manufacturing  variability and tolerances,  this assumption
is  straightforward  and hence  does  not  require further  justifications.  The
final assumption  from an  electrochemical point  of view  is that  the relative
thicknesses of each electrode is held constant to a fixed ratio. This assumption
requires further  explanation and is discussed  in \cref{sec:layeroptframework}.
Certain assumptions are to be made about the temperature distribution within the
layers  owing to  the choice  of cooling  arrangement and  is far  more involved
to  be  discussed  in  this  introductory section  and  are  also  discussed  in
\cref{sec:layeroptframework}.

\subsection{Broad overview and motivation}
...
Prima facie,  although it appears  that the  absolute lowest possible  number of
layers (\ie  one layer) is the  best choice for maximising  driving range, there
exist two other goals that conflict with this design intent. Firstly, there must
be a  minimum electrode  active surface  area to handle  the power  demands, and
hence, a minimum number of layers  typically far greater than one. Secondly, the
acceptable limit on  lifetime degradation of cells places an  upper bound on the
allowable temperature  rise during vehicle  operation. Increasing the  number of
layers has a  two-fold mitigating effect on the  temperature-rise experienced by
the cell.  The \emph{power density} within  each layer is diminished  due to the
increased  available surface  area,  leading to  reduced  ohmic heat  generation
within the cell.  With each layer requiring a Al-Cu  current-collector pair, the
number of heat conduction pathways increases linearly with the number of layers.
Thus, increasing the number of layers has a beneficial effect on pack lifetime.
...
capacity. Determining this saddle point needs  to be performed for a curated set
of power  input/output conditions to  the cell. This  niche problem has  not yet
been tackled and  motivates the need to  perform a careful design  study
...
\begin{figure}[p]
    ...
    \end{minipage}
\end{figure}
\subsection{Computational Framework}\label{sec:layeroptframework}
