...
work  since  it  is  nestled  deep   within  the  broader  horizon  of  electric
drivetrain optimisation. \Cref{fig:fig_PowertrainSchematic} provides a graphical
overview depicting the hierarchical  architecture of a typical \glsfmtshort{xeV}
powertrain, from  the system level down  to a single electrochemical  layer. The
rest of this section describes the scope of this layer optimisation work and its
integration into this overall architecture.
...
The   components   at   the  top   row   of   \cref{fig:fig_PowertrainSchematic}
represents  the  typical  constituents   of  a  \emph{series}-hybrid  powertrain
architecture~\cite{Emadi2017}. For partial supply of the mechanical power and/or
to partially charge the battery during propulsion, a downsized \gls{ice} coupled
to the pack's DC bus through  a generator and three-phase rectifier is employed.
While  tackling  the power  handling  requirements,  irrespective of  whether  a
\gls{bev} or  \gls{phev} powertrain is being  considered, the cells in  the pack
are to  be designed  for the  worst-case operating  scenario, \ie{}  without any
power support from the \gls{ice}. This implies that all discharge simulations of
the \gls{phev} are to be conducted with the powertrain operating in all-electric
mode  resulting in  a net  charge-depletion. The  only distinction  is that  the
\emph{magnitude} of power to be handled by  the pack in this worst case scenario
is vastly different between the \gls{bev}  and \gls{phev} cases. This allows for
some simplification  and helps  to narrow down  the scope of  the problem  to be
tackled as explained below.
...
presented here  in the context  of this  \gls{bev} powertrain. However,  being a
modular framework,  the optimisation  methodology may be  readily extended  to a
\gls{phev} powertrain and shall be discussed in \cref{sec:commonmodulelayeropt}.
...
pack is either  ---
\begin{enumerate*}
    \item by fast charging from  the mains~$P^\text{fastchg}_\text{batt}$ (charge), or
    \item acceleration $P^\text{acc}_\text{batt}$ (discharge) for  both the
        \acrshort{xeV} platforms and is computed from the power required at the
        wheels, $P_\text{w}$.
\end{enumerate*}

 % charge  being positive and discharge being negative.
% The \gls{phev} pack has  one string while the \gls{bev} pack has
% 3 parallel  strings, and  both have 8  series-connected modules.  Identical pack
% modules  are assumed  across  both xEV  platforms. Use  of  an identical  module
% facilitates  fixed exterior  geometry of  cells. Table  2 shows  all pouch  cell
% parameters. There  exist 12 series-connected  similar cells in each  module with
% battery  circuit symbols  as shown  in Fig.2  at its  left. The  \gls{phev} pack
% contains 33.33% of  the cells in the \gls{bev} pack,  whose architecture being a
% 96S-3P cell  assembly, leaving the \gls{phev}  pack to a 96S-1P  cell structure.
% The DC  bus voltage is unaltered  as both the  packs have same amount  of series
% cells. Uniform  distribution of  power is  assumed across  the cells  within the
% pack. The power required at the terminal  of the pack is computed. From these, a
% first -  order design ballparking of  the layers of  the cell is made  through a
% single cell simulation. This process  enables reduced simulation runtime and one
% cell conditions represent all. However  this assumption overlooks the likelihood
% of being  defeated in real  time automobile  operation. But the  modern advanced
% battery  management systems  aim  so  ( SOC,  Temperature  etc.) employing  pack
% balancing, temperature  governing mechanisms to validate  these assumptions. The
% cell to cell deviations is minimized for a controlled environment.
...
surface cooling,  and ii) as  little as33% of the  loss rate of  usable capacity
