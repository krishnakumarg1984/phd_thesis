...
\subsection{Pack-level --- strings, modules \& cells}
...
\Crefrange{eqn:defineChebNodes}{eqn:solidDiffEqChebDomain}   detail  the   steps
...
direction, are given by \cref{eqn:defineChebNodes}~\cite{Trefethen2000}.
...
for  each particle,  we  obtain  \cref{eqn:quotientappliedpde}, presented  along
with  its   Neumann  boundary   conditions.  $j$~is   the  molar   flux  density
(\si{mol.m^{-2}.s^{-1}}) and $R_\text{p}$ is the particle radius (\si{m}).
\begin{subequations}\label{eqn:quotientappliedpde}
    \begin{align}
        \frac{\partial c_\text{s}}{\partial t} &= D^\text{eff}_\text{s} \left( \frac{\partial}{\partial r} \frac{\partial c_\text{s}}{\partial r} + \frac{\partial^2 c_\text{s}}{\partial r^2} \right) \qquad r \in [0, R_\text{p}]\tag{\ref{eqn:quotientappliedpde}}\\
        \diffp{c_\text{s}}{r}{\mathrlap{r = 0}} &= 0\label{eqn:quotientappliedpdeBCcentre}\\
        D^\text{eff}_\text{s}\diffp{c_\text{s}}{r}{\mathrlap{r = R_\text{p}}} &= -j\label{eqn:quotientappliedpdeBCsurface}
    \end{align}
\end{subequations}
...
\crefrange{eqn:quotientappliedpde}{eqn:quotientappliedpdeBCsurface} whilst
retaining $c_\text{s}$ in the physical space yields
\crefrange{eqn:solidDiffEqChebDomain}{eqn:solidDiffEqChebDomainBCsurface}.

\begin{subequations}\label{eqn:solidDiffEqChebDomain}
    \begin{align}
	    \frac{\partial c_\text{s}}{\partial t}                         & = 4 \frac{D^\text{eff}_\text{s}}{R_\text{p}^2} \left( \frac{2}{\widetilde{r} + 1} \frac{\partial c_\text{s}}{\partial \widetilde{r}} + \frac{\partial^2 c_\text{s}}{\partial {\widetilde{r}}^2} \right)\tag{\ref{eqn:solidDiffEqChebDomain}} \\
        \diffp{c_\text{s}}{\widetilde{r}}{\mathrlap{\widetilde{r}=-1}} & = 0\label{eqn:solidDiffEqChebDomainBCcentre}                                                                                                                                                                                                                \\
        2 \frac{D^\text{eff}_\text{s}}{R_\text{p}} \diffp{c_\text{s}}{\widetilde{r}}{\mathrlap{\widetilde{r}=1}} & = -j\label{eqn:solidDiffEqChebDomainBCsurface}
    \end{align}
\end{subequations}

During  the iterative  solution process,  the spatial  gradients of  solid phase
lithium concentration  in \cref{eqn:solidDiffEqChebDomain} in this  case are not
...
by a  Chebyshev differentiation  matrix. This  particular aspect  is responsible
for  the inherent  reduction of  simulation  runtime achieved  by introducing  a
spectral method.  In the updated  version of  LIONSIMBA~v2.0 used in  this layer
optimisation  methodology, differentiation  matrices of  suitable dimensions  as
...
%Basically the pack configuration and the universe in which I worked shall be
...
%enable the creation of the universe in which I define the problem to be solved
