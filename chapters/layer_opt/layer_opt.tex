...
A  layer optimization  methodology in  3.1  is proposed  within the  categorized
powertrain  architecture  of a  typical  xEV  is shown  in  Fig.  2. Omit  those
components to the left of the battery  pack to decompose the xEV powertrain into
that for  a BEV. The anticipated  BEV powertrain comprises of  a). battery pack,
b). three  phase inverter,  c). Permanent Magnet  Synchronous Motor  (PSSM), d).
gearbox for torque multiplication, and e)  rest of the power train (differential
shaft  and driven  wheels). Fig.2  describes  all components  in the  powertrain
schematic and  if all these  components are  taken into consideration,  a series
plug-in-hybrid configuration is  attained. For partial supply  of the mechanical
power  and/or to  partially charge  the battery  during propulsion,  a downsized
Internal  Combustion Engine  (ICE), coupled  to the  pack’s DC  bus through  a
generator and three-phase rectifier can be  employed as envisaged. The cells are
designed for the  worst-case operating scenario, without any  power support from
the ICE, by conducting all discharge simulations of the PHEV with the powertrain
operating in all-electric mode, viz.  charge-depletion. The applied battery pack
power source is either fast charging from the mains P fastchg, batt ( Charge) or
acceleration P acc, batt (discharge) for  both the xEV platforms and is computed
from the power  required at the wheels, PW, charge  being positive and discharge
being negative. The PHEV  pack has one string while the BEV  pack has 3 parallel
strings, and  both have 8  series-connected modules. Identical pack  modules are
assumed across both xEV platforms. Use  of an identical module facilitates fixed
exterior geometry of cells. Table 2 shows all pouch cell parameters. There exist
12 series-connected similar cells in each module with battery circuit symbols as
shown in Fig.2  at its left. The PHEV  pack contains 33.33% of the  cells in the
BEV pack, whose architecture being a 96S-3P cell assembly, leaving the PHEV pack
to a 96S-1P  cell structure. The DC  bus voltage is unaltered as  both the packs
have  same amount  of series  cells. Uniform  distribution of  power is  assumed
across the cells within the pack. The power required at the terminal of the pack
is computed.  From these, a  first - order design  ballparking of the  layers of
the  cell  is made  through  a  single  cell  simulation. This  process  enables
reduced simulation runtime  and one cell conditions represent  all. However this
assumption overlooks  the likelihood of  being defeated in real  time automobile
operation. But  the modern  advanced battery  management systems  aim so  ( SOC,
Temperature etc.) employing pack  balancing, temperature governing mechanisms to
validate  these assumptions.  The cell  to cell  deviations is  minimized for  a
