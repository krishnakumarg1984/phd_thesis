...
the magnitudes to  forced air convection is used to  represent the heat transfer
from the cell to the environment. The heat exchange area is the combined surface
area of the  two cooling tabs that are  situated at either end of  the cell. The
temperature of the  coolant (otherwise called thermal sink) used  in the thermal
management system is denoted by  $T_\text{sink}$. In this work, this temperature
is  kept  constant  for  all  simulations.  The  bi-directionally  coupled  cell
temperature - Tcell(t) to the  electrochemical model changes to achieve variable
heat transfer between cell and coolant.  The cell’s constituents and the layer
configuration  decide  the lumped  specific  heat  capacity for  computation  as
explained in 3.1. Now, the P2D model  (modified to handle power inputs) as shown
in the  right foreground  of Fig.2, is  explained in 3.2.  Appendix A  gives the
relevant equations.



As a final observation, all layers within the cell are electrically in parallel,
which implies  that their  terminal voltage are  identical while  the externally
applied current (or power) is equally divided among the layers.
...
electrochemical aspects of the design simulation.
...
suitably scaled down to power-density inputs at the layer level.
