...
values that overlap between these two tables are the cut-off voltages and number
of nodes  for numerical  discretisation of  the governing  \gls{pdae} equations.
For  these  conflicting  quantities, \cref{tbl:lcoSimParamslayeropt}  is  chosen
to  prevail  for  all  simulation  studies in  this  chapter.  Furthermore,  the
individual  electrode  thicknesses  from  \cref{tbl:lcoSimParamsSPMp2d}  is  not
directly used,  but instead  calculated for  every layer  choice by  keeping the
ratio of their relative thicknesses constant.  This aspect shall be explained in
\cref{sec:layeroptframework}.

\Cref{fig:fig_CC_discharge_curves} illustrates  the influence  of the  number of
layers  on the  energy and  power  capability of  the example  cell. A  constant
current discharge  of \SI{60}{\ampere} is applied  to each of the  virtual cells
each set up with  a different layer configuration with a  total of five distinct
layer choices  carefully conjured  to provide  an illustration  the energy/power
...
Galvanostatic discharge of a
LiCoO2/graphite cell with  parameters as shown in table 2  are referred to model
predictions. The  terminal voltage  of the  cell is  maximum during  the initial
discharge  with  maximum layer  counts,  eg.  n =  {  90  ,70 },  comprising  of
comparatively lower resistances.  A reduced capacity compared to n  = 50 results
due to  inherent lower volume  of active  material. When n  = 10 ,  the terminal
voltage of the cell  diminishes with voltage cut-off keeping SOC  at 96%. As the
resistance of  the cells with lesser  layers is significantly high,  it leads to
high over potentials  with in the cell. The usable  energy under a galvanostatic
discharge for lesser  layers is compared in  Table. 1. It can be  seen from this
table that the usable energy which could be extracted is meagre for lesser layer
counts, however  high the theoretical  capacity Qn  is. Table 1  also highlights
that the C-rate of the cell varies during a galvanostatic discharge , due to the
dependence of its nominal capacity on the number of layers n. The ratio of power
to nominal capacity known as the E-rate varies with n for constant power inputs.
This is discussed in the remainder of this work. Considering all these factors a
reasonable  choice of  number of  layers ie.  n =  50, as  one of  the practical
compromises for the surface area available  for reaction and the total volume of
active material. This  selection of layer offers the peak  usable energy for the
given discharge  rate , out  of the  finite layer configurations.  The parabolic
relationship (downward  opening) between  the usable  energy of  a cell  and its
number of layers justifies the same.
