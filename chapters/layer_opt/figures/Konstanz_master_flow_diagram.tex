...
\begin{figure}[p]
    \begin{center}
        ...
        \begin{tikzpicture}[scale=0.78, every node/.style={scale=0.78},node distance = 7mm,>=Stealth,auto,
            ...
            % % \node[controlnode] at (searchterminatedcheckacc -| scalebysurfacearea) (bisectupwards) {$L=n^{(i)}+1$\\$n^{(i)} = n^{(i)}+1$};
            ...
            \draw[->,name path=failurepath] (acctempcheck.east) -- ++(7mm,0) node[midway] {\small no} |- node[above,pos=0.7] {\tiny Bisect} node[below,pos=0.75] {\tiny Downwards} (iterativesearch.west);
            ...
            % \draw[->,name path=failurepath3] (accsoccheck.east) -- ++(7.69mm,0) node[midway] {\small no} |- node[ near end] {\tiny Bisect } node[ near end,below] {\tiny Downwards} (iterativesearch.west);
            ...
            % \draw[name path=failurepathfastchg] (tempcheckfastchg.west) -- ++(-10mm,0) node[midway,above] {\small no} -| ++(0,-151.75mm) -| (bisectdownwards.south);
            ...
            \draw [->] (storefeasiblesolution) -- node[above,rotate=-90] {\tiny Bisect} node[below,rotate=-90] {\tiny Upwards} (storefeasiblesolution |- iterativesearch.south);
