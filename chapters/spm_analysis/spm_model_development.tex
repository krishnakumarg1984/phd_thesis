...
the simplest of  all time domain \glspl{pbm}. The rest  of this section provides
this  author's  digested  summary  of  its  modelling  rubrics  based  upon  the
keen  insights  gained from  perusing  the  vanguard  literature in  this  topic
\eg{}~\cite{Santhanagopalan2006,Santhanagopalan2006a,DiDomenico2010} as  well as
the derivative works based upon them.
...
\begin{figure}[!htbp]
    ...
    {%
        ...
        dimension along the axial direction. Image reproduced
...
all solid phase diffusivities and  kinetic parameters are held constant. Thermal
effects are assumed to be negligible and no degradation effects are attempted to
be modelled in the basic \gls{spm} formulation.
...
cell's dynamics arising  due to solid phase diffusion and  reaction kinetics, in
...
and the  azimuthal axes, the  divergence becomes a  function of only  the radial
position. \Cref{eq:fullsphericaldiv} therefore reduces to
...
noted  that  the  term  involving  the Faraday  constant  in  the  \gls{rhs}  of
\cref{eq:uniformcurrdensity}  is~$nF$,  where  $n$~is the  number  of  electrons
...
omitted for all potential occurrences.
...
electrode is used for this computation.
...
provide acceptable  levels of  performance for both  transient and  steady state
...
electrolyte    potential    to    the   overpotential    is    neglected    (see
\cref{subsec:basicspmgeometry,subsec:basicspmassumptions} for a brief discussion
on the exclusion of electrolyte dynamics).
...
\ie{}   $\diff{n_\text{Li}}{t}   =   0$.    Substituting   this   condition   in
...
The  measured   variable~\mbox{$y  ∈  \mathbb{R}$}  is   the  cell's  terminal
...
\crefrange{eq:csposbulkfromcsnegbulk}{eq:csurfnegfromcavgneg}  form  a  complete
