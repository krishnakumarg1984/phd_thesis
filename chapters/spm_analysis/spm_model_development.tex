...
the  simplest of  all  time domain  \glspl{pbm}  and the  rest  of this  section
provides an expository treatment of its modelling rubrics.

\subsection{Geometry}\label{subsec:basicspmgeometry}

\Cref{fig:sandwichtospm} shows  the zoomed-in view of  one electrochemical layer
within  a  typical  lithium  ion  pouch  cell.  A  description  of  the  working
principle of the  cell was presented in \cref{subsec:liionchemistry}  and is not
repeated here. The  \gls{spm}, as the name suggests,  models the electrochemical
phenomena along the thickness $l_j$, \jinnegpos{}  of each porous electrode by a
