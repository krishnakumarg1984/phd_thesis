...
\chapter[%
Computational Analysis and Numerical Reformulation of the \glsfmtshort{dra}
]%
{%
Computational     Analysis     and     Numerical    Reformulation     of     the
\glsfmtshort{dra}\footnote{This chapter is based  on the journal publication ---
...
This author wishes to express his  thankfulness to Teng Zhang, co-author of this
article,  for checking  my  calculations and  providing  valuable feedback  that
helped to refine the manuscript for that journal publication.}
}\label{ch:improveddra}
...
\newpage
\capolettera{T}{his}  chapter  presents  an  analysis  and  critical  evaluation
...
which is available in the textbook by Plett~\cite{Plett2015}.}.  \Cref{sec:Analysis-of-the}
...
computational time of  the \gls{rom}  workflow. \Cref{sec:Results}
...
of  the  \glsfirst{p2d}  volume-averaged  \gls{p2d} implementation of the continuum  model  proposed  by  Doyle,
...
\begin{equation}\label{eq:blockhankel}
    ...
    \end{bmatrix}
\end{equation}
where $g_k$ represents the  Markov parameter of the system at time-index $k$.
...
A  key  computation  in  the  Ho-Kalman  algorithm  is the  \glsfirst{svd}  of
...
\begin{figure}[!htbp]
    ...
    \caption[%
    \glsfmtshort{rom} workflow using the classical \glsfmtshort{dra} scheme
    ]%
    {%
...
\begin{figure}[!htbp]
    ...
    \caption[%
    \glsfmtshort{rom} workflow using the improved \glsfmtshort{dra} scheme
    ]%
    {%
...
\begin{table}[!htbp]
	% \singlespacing
	\centering
	\caption{Salient Results --- Classical vs.\ Improved \glsfmtshort{dra} }
